...
#'
#' @param x a list of dataframes
#' @param update logical. whether or not to update clone codes
#'
#' @return a list of dataframes
#' @export
#'
#' @examples
pre_process_trials <- function(x, update = TRUE){
  if(isTRUE(update)){
    out <- purrr::map(x, update_geno)
  }else{
    out <- x
  }
  out <- keep_geno(out) %>%  purrr::map(., rename_cols) %>%
...
#'
#' @param clones ca dataframe with `geno` column
#' @param tot integer. total number of unique clones/genotypes to be randomized to field
#' @param loc character. trial location
#' @param rowD integer. number of rows in the field
#' @param n_dummies integer. number of dummies to complete a rectangular layout
...
randomize_row_col <- function(clones,
  ...
  if (!is.null(path)) {
    png(
      file.path(path, season, "fieldbook",  paste0(loc, ".png")),
      width = 2000,
      height = 1500,
      res = 150
    )
    ...
    readr::write_csv(design,
                     file.path(path, season, "fieldbook",  paste0(loc, ".csv")))
  }
  return(design)
}
```
