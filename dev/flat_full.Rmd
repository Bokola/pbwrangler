  ...
  out <- out %>% purrr::map(
    ., ~dplyr::mutate(
      ...
      planting_date = dplyr::case_when(
        !grepl("\\d+-", s)  ~ as.Date(paste0("01-04-", sub("^([^-|^ ])*.", "", s) %>%
                                                 gsub("-.*", "",.)), format = "%d-%m-%Y"),
        TRUE ~ as.Date(paste0("01-11-", sub("^([^-|^ ])*.", "", s) %>%
                                gsub("-.*", "",.)), format = "%d-%m-%Y")
      ),
      
      # planting_date = as.Date(paste0("01-04-", sub("^([^- | ^ ])*.", "", s) %>%
      #                                            gsub("-.*", "",.)), format = "%d-%m-%Y"),
      ...
      # harvest_date = as.Date(paste0("01-04-",sub("^([^- | ^ ])*.", "", s) %>%
      #                                            gsub("-.*", "",.)), format = "%d-%m-%Y") + 100
    )
  )
...
write_season_data <- function(list_df, season){
  ...
  outlying <- outlying[!sapply(outlying, is.null)]
  outlying <- outlying[sapply(outlying, function(x) nrow(x) > 0)]
  ...
  function(x,
    ...
    y <-
      st4gi::cdt(
        ...
        method = "ps",
        (
          readr::parse_number(x$space_between_plants_in_ridges) * readr::parse_number(x$space_between_ridges)
...
rename_cols <- function(x) {
  x <- x %>%  dplyr::rename_with(
    ~ dplyr::case_when(
      ...
      grepl("plot_id|plot_no", ., ignore.case = TRUE) ~ "plot",
...
#' @param btwn spacing between ridges in metres
#' @param within spacing between plants in the same ridge in metres
#' @param crop one of \code{'pt'} or \code{'sp'}
#'
#' @return a dataframe
#' @export
#'
...
#' @param merge logical. whether or not to merge data from multiple sheets 
#' @param update_plot logical. whether or not to update plot column to conform
#'  to SOP. This applies to trials preceeding season 2025.
#'
...
read_workbooks <- function(dir = t_dir
  ...
  "weight_g_10_30"
    ...
    if (isTRUE(update_plot)) {
      out <- out %>% capture_location() %>%
        purrr::map(., ~ dplyr::mutate(., plot = paste0(
          loc, "-", stringr::str_pad(plot, 5, pad = "0")
        ))) %>% purrr::map(., ~ dplyr::select(., -loc))
    }
